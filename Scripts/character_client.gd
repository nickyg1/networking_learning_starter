...
func _ready() -> void:
	print("spawing client script %s" % character_network.owner_id)
	character_network.on_ownership_change.connect(_on_ownership_changed)
...
func _input(event: InputEvent) -> void:
	if !is_instance_valid(my_camera) || !character_network._is_owner(): return
